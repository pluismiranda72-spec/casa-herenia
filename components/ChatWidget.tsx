...
export default function ChatWidget() {
  ...
  const {
    ...
    body: {},
    onError: (err) => {
      console.warn("[Aura]", err?.message ?? "Error de conexión con el asistente.");
    },
  });
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-[1000] flex flex-col items-end gap-2">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && !error && (
              <p className="font-sans text-sm text-brand-black/60 text-center py-4">
                Hola. Soy Aura. Pregúntame sobre check-in, wifi, desayuno o la
                casa.
              </p>
            )}
            {error && (
              <p className="font-sans text-sm text-red-600/80 text-center py-2">
                Algo ha fallado. Intenta de nuevo en un momento.
              </p>
            )}
            {messages.map((m) => (
              <div
      ...
      <button
        ...
        aria-label={open ? "Cerrar chat" : "Abrir chat con Aura"}
      >
