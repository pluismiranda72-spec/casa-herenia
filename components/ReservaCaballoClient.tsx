...
export default function ReservaCaballoClient() {
  ...
  const [caballoBlockedDates, setCaballoBlockedDates] = useState<Set<string>>(new Set());

  useEffect(() => {
    fetch("/api/tours/caballo/availability")
      .then((res) => res.json())
      .then((data: { blocked?: string[] }) => {
        setCaballoBlockedDates(new Set(data.blocked ?? []));
      })
      .catch(() => {
        setCaballoBlockedDates(new Set());
      });
  }, []);
  ...
  const handleCheckout = async () => {
    try {
      setIsLoading(true);
      const res = await fetch("/api/checkout-caballo", {
        method: "POST",
  ...
  return (
    <div className="w-full max-w-3xl px-4">
      ...
      >
        Disponibilidad: Ruta a Caballo en Viñales
      </h1>
      <div
      ...
      >
        ...
        <span className="flex flex-col md:flex-row md:items-center gap-2 md:gap-3">
          <span className="flex items-center gap-1.5">
            ...
            {locale === "en" ? "Guided route" : "Ruta guiada"}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="text-sky-500 font-bold text-lg leading-none">•</span>{" "}
            {locale === "en" ? "Horse" : "Caballo"}
          </span>
        </span>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 md:p-6 shadow-sm flex flex-col md:flex-row md:items-start md:gap-8">
          <div className="w-full md:flex-1 min-w-0 flex justify-center">
            <DayPicker
              ...
              disabled={(date) => caballoBlockedDates.has(dateToKey(date))}
